...
var OmniTrackerSettingsTab = class extends import_obsidian.PluginSettingTab {
...
function displayTracker(app, tracker, element, getFile, getSectionInfo, settings, component) {
  ...
  if (activeEntry) {
    new import_obsidian3.ButtonComponent(controls).setClass("clickable-icon").setIcon(`lucide-${runningEntry ? "pause" : "play"}-circle`).setTooltip(runningEntry ? "Pause" : "Resume").onClick(() => __async(this, null, function* () {
      if (runningEntry) {
    ...
    new import_obsidian3.ButtonComponent(controls).setClass("clickable-icon").setIcon("lucide-stop-circle").setTooltip("End").onClick(() => __async(this, null, function* () {
      endRunningEntry(tracker);
      yield saveTracker(app, tracker, getFile(), getSectionInfo());
    })).buttonEl.addClass("omnitracker-btn");
  } else {
    new import_obsidian3.ButtonComponent(controls).setClass("clickable-icon").setIcon("lucide-play-circle").setTooltip("Start").onClick(() => __async(this, null, function* () {
      startNewEntry(tracker, newSegmentNameBox.getValue());
      yield saveTracker(app, tracker, getFile(), getSectionInfo());
    })).buttonEl.addClass("omnitracker-btn");
  }
  let newSegmentNameBox = new import_obsidian3.TextComponent(element).setPlaceholder("Segment name").setDisabled(!!activeEntry);
...
function getDuration(entry) {
  if (entry.subEntries) {
    ...
    for (let interval of entry.intervals) {
      let start = (0, import_obsidian3.moment)(interval.startTime);
      let end = interval.endTime ? (0, import_obsidian3.moment)(interval.endTime) : (0, import_obsidian3.moment)();
...
function getDurationToday(entry) {
  if (entry.subEntries) {
    ...
    let total = 0;
    for (let interval of entry.intervals) {
      let startTime = (0, import_obsidian3.moment)(interval.startTime);
      let endTime = interval.endTime ? (0, import_obsidian3.moment)(interval.endTime) : (0, import_obsidian3.moment)();
...
function getPausedEntry(entries) {
  for (let entry of entries) {
    if (entry.subEntries) {
...
function startNewEntry(tracker, name) {
  ...
  let now = (0, import_obsidian3.moment)().toISOString();
  let entry = {
    name,
    startTime: now,
    endTime: null,
...
function pauseEntry(entry) {
  if (entry.intervals && entry.intervals.length > 0) {
    let last = entry.intervals[entry.intervals.length - 1];
    if (!last.endTime) {
      last.endTime = (0, import_obsidian3.moment)().toISOString();
    }
  }
  entry.paused = true;
}
function resumeEntry(entry) {
  if (!entry.intervals) {
    if (entry.startTime) {
      entry.intervals = [{ startTime: entry.startTime, endTime: entry.endTime || (0, import_obsidian3.moment)().toISOString() }];
    } else {
      entry.intervals = [];
    }
  }
  entry.intervals.push({ startTime: (0, import_obsidian3.moment)().toISOString(), endTime: null });
  entry.paused = false;
}
...
function addEditableTableRow(app, tracker, entry, table, newSegmentNameBox, trackerRunning, getFile, getSectionInfo, settings, indent, component) {
  ...
  let expandButton = new import_obsidian3.ButtonComponent(nameField.label).setClass("clickable-icon").setClass("omnitracker-expand-button").setIcon(`chevron-${entry.collapsed ? "left" : "down"}`).onClick(() => __async(this, null, function* () {
    if (entry.collapsed) {
  ...
  if (entryActive) {
    new import_obsidian3.ButtonComponent(entryButtons).setClass("clickable-icon").setIcon(`lucide-${entryRunning ? "pause" : "play"}`).setTooltip(entryRunning ? "Pause" : "Resume").onClick(() => __async(this, null, function* () {
      if (entryRunning) {
        pauseEntry(entry);
      } else {
        resumeEntry(entry);
    ...
    new import_obsidian3.ButtonComponent(entryButtons).setClass("clickable-icon").setIcon("lucide-square").setTooltip("End").onClick(() => __async(this, null, function* () {
      endRunningEntry(tracker);
      yield saveTracker(app, tracker, getFile(), getSectionInfo());
    }));
  } else {
    new import_obsidian3.ButtonComponent(entryButtons).setClass("clickable-icon").setIcon("lucide-play").setTooltip("Continue").setDisabled(trackerRunning && !entryRunning).onClick(() => __async(this, null, function* () {
...
// src/main.ts
var OmniTrackerPlugin = class extends import_obsidian4.Plugin {
  constructor() {
    super(...arguments);
    this.api = {
      loadTracker,
